...
async function rewriteWithAI(title: string, content: string) {
    ...
    ### BRANDING RULE:
    - You MUST replace any mention of "Investing.com" with "Bond.az" in all titles and content. No other source names should be mentioned.
...
const r2AccessKeyId = process.env.CLOUDFLARE_R2_ACCESS_KEY_ID || process.env.R2_ACCESS_KEY_ID;
const r2SecretAccessKey = process.env.CLOUDFLARE_R2_SECRET_ACCESS_KEY || process.env.R2_SECRET_ACCESS_KEY;
...
async function fetchWithViewSource(url: string) {
    ...
    try {
        ...
        // Expected URL: https://www.investing.com/news/economy-news/title-123
        if (urlParts.length >= 5 && urlParts[3] === 'news') {
            catSlug = urlParts[4].replace('-news', ''); // economy-news -> economy
            // Create a readable name from slug (e.g., stock-market -> Stock Market)
            catName = catSlug.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
        }
        
        // Extract Title from requested meta/title tag
        const pageTitle = $('title[data-next-head=""]').text().replace(' By Investing.com', '').trim();
        
        // Extract Content from requested div structure
        const contentDiv = $('#article [class*="article_WYSIWYG"]');
        const pTexts = contentDiv.find('p').map((_, el) => $(el).text().trim()).get().filter(t => t && !t.includes('Get more cues on Bitcoin'));
        
        const ogImage = $('meta[property="og:image"]').attr('content') || $('#article img').first().attr('src');
...
async function runNewsBot(limit = 5) {
    console.log(`--- INVESTING AI BOT: STARTING TOP-${limit} CHECK ---`);
    console.time("Total Action Time");

    try {
        const feed = await parser.parseURL('https://www.investing.com/rss/news.rss');
        const itemsToCheck = feed.items.filter(item => !item.link?.includes('/news/pro/')).slice(0, limit);
        let publishedCount = 0;

        for (const item of itemsToCheck) {
            const commonId = item.guid || item.link;
            // Check if post already exists by common_id (GUID or Link)
            ...
            try {
                ...
                const targetTitle = source.title || item.title || "";
                const [aiResult, generatedImageUrl] = await Promise.all([
                    rewriteWithAI(targetTitle, source.text),
                    generateImageWithOpenAI(targetTitle)
                ]);

                // Upload to R2 (Fallback to original if AI generation fails)
                const myImageUrl = await uploadImageToR2(generatedImageUrl || source.imageUrl!, targetTitle);
